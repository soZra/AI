...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        self.a_levels.append(set())     # A(n) set of a_nodes - empty to start 创建A(n)集合


        for actions in self.all_actions:        # 所有动作循环一遍
            Action_Node = PgNode_a(actions)              #创建动作结点
            IsVaild = True                      
            for Pre_S in Action_Node.prenodes:        # 外层循环  判断动作结点是否可添加
                flag = False
                for Level_S in self.s_levels[level]:        # 内层循环  判断动作结点前提状态是否全部满足
                    if Pre_S.__eq__(Level_S):
                        flag = True
                        Action_Node.parents.add(Level_S)             # 将上一层状态结点作为父节点添加到动作结点集合中
                        break
                if flag is False:
                    IsVaild = False
                    break

            if IsVaild is True:                     # 若动作结点可添加
                for node in Action_Node.prenodes:        # 将动作结点添加到上一层相应状态结点的子节点集合中
                    node.children.add(Action_Node)
                self.a_levels[level].add(Action_Node)        #  往动作层加动作结点
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        for node_1 in node_a1.effnodes:         # 双重循环检查两个行动结点所有效果结点，若有冲突立即返回
            for node_2 in node_a2.effnodes:
                if self.negation_mutex(node_1,node_2):
                    return True
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        for node_1 in node_a2.effnodes:
            for node_2 in node_a1.prenodes:
                if self.negation_mutex(node_1,node_2):
                    return True
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        for node_1 in node_a1.parents:         # 双重循环检查两个行动结点所有前置条件结点，若有冲突立即返回
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        for action_node1 in node_s1.parents:
            for action_node2 in node_s2.parents:
                if action_node1.__eq__(action_node2):
                    return False


        for action_node1 in node_s1.parents:                #  对于每个外层循环的动作结点，都有一个内层循环的动作结点与之冲突
            is_mutex = False
            for action_node2 in node_s2.parents:
                if action_node1.is_mutex(action_node2):
                    is_mutex = True
                    break
            if is_mutex is False:
                return False
    ...
    def h_levelsum(self) -> int:
        ...
        for goal in self.problem.goal:
            Is_goal_happen = False
            level = 0
            while not Is_goal_happen:
